...
其中 $W^{Q}\_{i} \in \mathbb{R}^{d\_{model}\times{d\_k}}$ ，也就是我们前文提到的Wq，Wk和Wv也是类似，而 $W^{O} \in \mathbb{R}^{hd\_v \times{d\_{model}}} $ 。为了加快计算速度，可以并行计算每个head，并且减少每个head的维度。在论文中，作者设置h=8，并且$ d_k=dv=d_{model}/h=64 $ 。
